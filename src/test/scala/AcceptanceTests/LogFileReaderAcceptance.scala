...
class LogFileReaderAcceptance extends FlatSpec with Matchers {
  ...
  def compareActualToExpected(file:File, friendlyHand: List[Card] = Nil, friendlyBoard: List[Card] = Nil, enemyHand: List[Card] = Nil, enemyBoard: List[Card] = Nil, friendlyWeaponValue: Option[Int] = None, enemyWeaponValue: Option[Int] = None): Unit = {
    ...
    val logFileReader = new LogFileReader(lp, actualGameState, file)
    logFileReader.read()
  ...
  it should "Detect friendly and enemy Hex" in {
    ...
    val expectedFriendlyBoard = List[Card](
      new Card("Transformed Minion", 70, 500, 2, 1, Constants.STRING_UNINIT),
    ...
    val expectedEnemyBoard = List[Card](
      new Card("Transformed Minion", 71, 500,1,2,Constants.STRING_UNINIT)
    )


    compareActualToExpected(new File(getClass.getResource("/debugsituations/Hex.txt").getPath), expectedFriendlyHand, expectedFriendlyBoard,expectedEnemyHand, expectedEnemyBoard)
  }
  ...
  it should "Detect weapon destroyed" in {
    ...
    val actualGameState = new GameState
    val lp = new LogParser(actualGameState)
    val file = new File(getClass.getResource("/debugsituations/WeaponsDestroyed.txt").getPath)
    val logFileReader = new LogFileReader(lp, actualGameState, file)
    logFileReader.read()
    logFileReader.updateGameState()
  ...
  it should "Detect when a new Hero Power is assigned" in {
    ...
    val friendlyHP = Card("Friendly Hero Power", 74, Constants.INT_UNINIT, Constants.INT_UNINIT, 1,"AT_132_PRIEST", cardInfo = actualGameState.getCardInfo("AT_132_PRIEST"))
    val enemyHP = Card("Enemy Hero Power", 67, Constants.INT_UNINIT, Constants.INT_UNINIT, 2,"CS2_017", cardInfo = actualGameState.getCardInfo("CS2_017"))
